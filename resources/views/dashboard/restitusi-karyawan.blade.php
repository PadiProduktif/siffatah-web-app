                            ...
                            <form  action="/admin/restitusi_karyawan/update/{{ $data1->id_pengajuan }}" method="POST">
                                ...
                                @php
                                    if (auth()->user()->role === 'dr_hph' || auth()->user()->role === 'vp_osdm' || $data1->status_pengajuan === 4) {
                                        $form_jenis_perawatan = "disabled";
                                        if(auth()->user()->role === 'dr_hph' || auth()->user()->role === 'superadmin' || auth()->user()->role === 'adm_karyawan'){
                                            $form_jenis_perawatan = "";
                                        }
                                ...
                                <div class="modal-body">
                                    <div class="row text-start">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-control" id="kategori_perawatan" name="kategori_perawatan" {{ $form_jenis_perawatan }}>
                                                <option value="" disabled>Pilih kategori perawatan</option>
                                                <option value="rawat_inap" {{ $data1->jenis_perawatan == 'rawat_inap' ? 'selected' : '' }}>Rawat Inap</option>
                                                <option value="rawat_jalan" {{ $data1->jenis_perawatan == 'rawat_jalan' ? 'selected' : '' }}>Rawat Jalan</option>
                                                <option value="operasi" {{ $data1->jenis_perawatan == 'operasi' ? 'selected' : '' }}>Operasi/Tindakan</option>
                                                <option value="asuransi" {{ $data1->jenis_perawatan == 'asuransi' ? 'selected' : '' }}>Asuransi</option>
                                ...
                                @if (auth()->user()->role === 'superadmin' && $data1->status_pengajuan === 1|| auth()->user()->role === 'adm_karyawan' && $data1->status_pengajuan === 1 || auth()->user()->role === 'tko' && $data1->status_pengajuan === 1 || auth()->user()->role === 'dr_hph' && $data1->status_pengajuan === 2)
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row mt-3">
                                <div class="col-md-12 text-end">
                                    <label for="totalKeseluruhanEstimasi" class="form-label fw-bold">Total Estimasi Keseluruhan:</label>
                                    <input type="text" id="totalKeseluruhanEstimasi" class="form-control text-end fw-bold" readonly>
                                </div>
                            </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-control" id="kategori_perawatan" name="kategori_perawatan">
                                ...
                                <option value="operasi_tindakan">Operasi/tindakan</option>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    response.data.forEach((item, index) => {
                        $(pengajuanBiayaKaryawanWrapper).append(`
                            <div class="row mb-2" id="pengajuanBiayaKaryawanRow-${index}">
                                ...
                                <div class="col-md-2">
                                    <label for="keterangan_pengajuan" class="form-label">Persetujuan Dokter</label>
                                ...
                                <div class="col-md-2">
                                    <label for="total_estimasi" class="form-label">Total Estimasi</label>
                                    <input type="text" class="form-control total_estimasi_item" readonly>
                                </div>
                    ...
                    $(document).on('change input', '.presentase, .nominal_dokter', function () {
                        let totalKeseluruhan = 0;

                        $('#pengajuanBiayaKaryawanWrapper .row').each(function () {
                            let nominalKaryawan = $(this).find('.nominal_karyawan').val().replace(/[^\d]/g, '');
                            let nominalDokter = $(this).find('.nominal_dokter').val().replace(/[^\d]/g, '');
                            let persentase = $(this).find('.presentase').val();
                            let totalEstimasiItemField = $(this).find('.total_estimasi_item'); // Target field estimasi per item

                            // Pilih nilai yang akan digunakan: nominalDokter jika ada, jika tidak gunakan nominalKaryawan
                            let nominalDigunakan = nominalDokter ? parseFloat(nominalDokter) : parseFloat(nominalKaryawan);

                            // Pastikan nilai valid sebelum melakukan perhitungan
                            let hasilEstimasiItem = 0;
                            if (!isNaN(nominalDigunakan) && !isNaN(persentase)) {
                                hasilEstimasiItem = (nominalDigunakan * parseFloat(persentase)) / 100;
                                totalKeseluruhan += hasilEstimasiItem;
                            }

                            // Update total estimasi per item
                            totalEstimasiItemField.val(`Rp ${hasilEstimasiItem.toLocaleString('id-ID')}`);
                        });

                        // Update total keseluruhan estimasi
                        $('#totalKeseluruhanEstimasi').val(`Rp ${totalKeseluruhan.toLocaleString('id-ID')}`);
                    });
                    $(document).on('show.bs.modal', '#approveDRModal', function () {
                        // Panggil event listener untuk menghitung ulang estimasi saat modal dibuka
                        $('.presentase, .nominal_dokter').trigger('change');
                    });
                }
            },
            error: function () {
