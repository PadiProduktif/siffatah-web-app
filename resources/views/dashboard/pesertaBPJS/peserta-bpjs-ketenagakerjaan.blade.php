...
@section('content')
    ...
    <div class="container">
        <div class="d-flex justify-content-end align-items-center mb-4">
            <h4 class="me-auto">Kepesertaan BPJS - Ketenagakerjaan</h4> <!-- Tambahkan kelas 'me-auto' untuk memberi margin ke kanan pada judul -->
    ...
    <table id="tableAdmin" class="display">
        ...
        <tbody>
            ...
            <tr class="detailRow"
                data-id="{{ $data->id_peserta_bpjs_ketenagakerjaan }}" 
                data-id_badge="{{ $data->id_badge }}" 
                data-nama_karyawan="{{ $data->nama_karyawan }}" 
                data-nik="{{ $data->nik }}"
                data-tgl_lahir="{{ format_date_locale($data->tgl_lahir)}}"
                data-alamat="{{ $data->alamat }}"
                data-no_bpjs="{{ $data->no_bpjs }}"
                data-file_url="{{ $data->file_url }}"> <!-- Tambahkan file_url -->
                <td><input type="checkbox" class="rowCheckbox" value="{{ $data->id_peserta_bpjs_ketenagakerjaan }}"></td>
                <td>{{ $data->id_badge }}</td>
                <td>{{ $data->nama_karyawan }}</td>
                <td>{{ $data->nik }}</td>
                <td>{{ format_date_locale($data->tgl_lahir) }}</td>
                <td>{{ $data->no_bpjs }}</td>
                <td>{{ $data->alamat }}</td>
                ...
                <td>
                    ...
                    <button class="btn btn-danger btn-sm deleteBtn" data-id="{{ $data->id_peserta_bpjs_ketenagakerjaan }}">Hapus</button>
    ...
    <div class="modal fade" id="modalDataBaru" tabindex="-1" aria-labelledby="modalDataBaruLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <form id="dataForm" action="{{ route('bpjs-ketenagakerjaan.store') }}" method="POST" enctype="multipart/form-data" >
                    @csrf
                    <div class="modal-body">
                        <div class="row g-3">
                            <!-- Informasi Dasar -->
                            ...
                            <div class="col-md-8">
                                <label for="gelar_belakang" class="form-label">No BPJS</label>
        ...
        <div class="modal fade" id="modalDataExcel" tabindex="-1" aria-labelledby="modalDataExcelLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                    <form action="{{ route('bpjs-ketenagakerjaan.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
        ...
        if (rowData.file_url) {
            ...
            if (files.length > 0) {
                attachmentHtml = files.map(file => {
                    ...
                    const filePath = `/uploads/PesertaBPJS/BPJS_Ketenagakerjaan/${file}`;
    ...
    $(document).ready(function () {
        ...
        $('#tableAdmin').on('click', '.editBtn', function (event) {
            ...
            $('#editForm').attr('action', '/admin/bpjs/bpjs-ketenagakerjaan/update/' + id);
    ...
    $('#tableAdmin').on('click', '.deleteBtn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                url: '/admin/bpjs/bpjs-ketenagakerjaan/delete/' + id,
            ...
            $('#deleteSelected').on('click', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/admin/bpjs/bpjs-ketenagakerjaan/delete-multiple', // Endpoint untuk hapus data
                            type: 'POST',
                            data: {
...
// let uploadedFiles = []; // Array untuk menyimpan nama file yang diunggah
        ...
        if (document.querySelector('#attachmentDropzone')) {
            const attachmentDropzone = new Dropzone("#attachmentDropzone", {
                url: "/bpjs-ketenagakerjaan/upload-temp", // Endpoint sementara untuk upload
                paramName: "file",
                headers: {
                    'X-CSRF-TOKEN': token
                },
                ...
                init: function () {
                    ...
                    this.on("removedfile", function (file) {
                        ...
                        if (file.uploadedFileName) {
                            ...
                            $.ajax({
                                url: "/bpjs-ketenagakerjaan/delete-temp",
                                type: "POST",
                                data: {
                                    _token: token,
        ...
        let editAttachmentDropzone = new Dropzone("#editAttachmentDropzone", {
            url: "/bpjs-ketenagakerjaan/upload-temp", // Endpoint sementara untuk upload file
            paramName: "file",
            headers: {
            ...
            init: function () {
                ...
                if (Array.isArray(existingFiles)) {
                    existingFiles.forEach(file => {
                        ...
                        this.emit("thumbnail", mockFile, `/uploads/PesertaBPJS/BPJS_Ketenagakerjaan/${file}`);
